...
function createHarness(nowMs, seed = {}) {
  ...
  const context = vm.createContext({
    ...
    state: {
      ...
      cardr: { vitals: [], procedures: {}, chips: {}, exams: {} },
      car: {}, problems: [], destination: {}
    },
...
async function main() {
  ...
  assert(serviceWorker.includes("const CACHE_PREFIX = 'doctorcar-pwa-'"));
  assert(serviceWorker.includes("doctorcar-pwa-v3.9.6-metrics-stop-20260908"));
  assert(serviceWorker.includes('k.startsWith(CACHE_PREFIX) && k !== CACHE_NAME'));
  ...
  queuedBeforeSend.forEach(item => {
    ...
    assert.strictEqual(item.payload.app, 'drcar');
  ...
  console.log('OK: only metrics storage keys are removed; doctorCarV1 and unrelated settings remain');
  console.log('OK: service worker cleanup is limited to Doctor Car cache keys');
}
